...
def blobs_to_windows(blobs, windows):
    new_windows = []
    for blob in blobs:
        y, x, r = blob
        startx = x
        ...
        for window in windows:
            ...
            if x >= startx1 and x <= endx1 and y >= starty1 and y <= endy1:
                startx = min(startx, startx1)
                starty = min(starty, starty1)
                endx = max(endx, endx1)
                endy = max(endy, endy1)
        ...
        if startx - starty != 0:
            new_windows.append(((int(startx), int(starty)), (int(endx), int(endy))))
...
def combine_boxes(windows, image_shape, max_sigma=200, threshold=0.08):
    if len(windows)>0:
        ...
        if np.sum(hot_windows)>0:
            ...
            return blobs_to_windows(blobs, windows)
